  ...
  return (
    <div className="pb-32 px-5 max-w-2xl mx-auto">
      ...
      <div className="space-y-10">
        <section className="space-y-4"><div className="flex items-center gap-2"><Crown size={14} className="text-orange-500" /><h3 className="text-[10px] font-black text-slate-500 uppercase italic">PRONOSTICS ELITE</h3></div>
            ...
            <div className="space-y-4">
              {data.predictions.map((p, i) => (
                <div key={i} className="bg-[#0b1121] p-8 rounded-[2.5rem] flex justify-between items-center shadow-xl border border-white/5 overflow-hidden relative">
                  ...
                  <div><span className="text-[10px] font-bold text-blue-400 uppercase block mb-1">{p.type}</span><div className="text-2xl font-black text-white italic uppercase">{p.recommendation}</div></div>
                  <div className="text-right"><div className="text-4xl font-black text-white">{p.probability}%</div><div className="flex items-center justify-end gap-1.5 mt-1"><div className={`w-2 h-2 rounded-full ${p.confidence === 'HIGH' ? 'bg-emerald-500' : 'bg-orange-500'}`}></div><span className="text-[9px] font-black text-slate-500 uppercase">{p.confidence === 'HIGH' ? 'ÉLEVÉ' : 'MOYEN'}</span></div></div>
                </div>
              ))}
            </div>
...
const App: React.FC = () => {
  ...
  const isVipRoute = location.pathname.includes('/vip');
